/*
 * SPDX-License-Identifier: GPL-3.0-or-later
 */
// 基于 ExiaInvasion 的结构，提供 zh/en 文案；可按需逐步扩展
export type Lang = 'zh' | 'en'

const TRANSLATIONS: Record<Lang, Record<string, string>> = {
  zh: {
    // 通用
    appTitle: 'ExiaAnalysis',
    zh: '中文',
    en: '英语',
    'lang.zh': '中文',
    'lang.en': 'English',
    'accountList.exportWithPlans': '导出规划',
    'accountList.exportWithPlansTitle': '导出当前账号列表及联盟突袭规划 Excel',
    'accountList.importNoAccounts': 'Excel 中未找到有效的账号数据',
    'accountList.importPlanningSuccess': '已导入联盟突袭规划，匹配 {matched} 个账号',
    'accountList.importPlanningPartial': '已导入联盟突袭规划，匹配 {matched} 个账号，跳过 {unmatched} 个未匹配账号',
    'accountList.importPlanningNoMatches': '上传的联盟突袭规划未匹配任何当前账号',
    'accountList.exportNoData': '暂无账号可导出',
    'accountList.exportSuccess': '已成功导出带联盟突袭规划的 Excel',
    'accountList.exportFailed': '导出 Excel 失败',

    // 认证
    'auth.login': '登录',
    'auth.logout': '退出',
    'auth.logoutSuccess': '已退出',
    'auth.register': '注册',
    'auth.titleLogin': '登录',
    'auth.titleRegister': '注册',
    'auth.username': '用户名',
    'auth.password': '密码',
    'auth.submitLogin': '登录',
    'auth.submitRegister': '注册',
    'auth.switchToRegister': '去注册',
    'auth.switchToLogin': '去登录',
    'auth.cancel': '取消',
    'auth.required': '请填写用户名和密码',
    'auth.usernameRequired': '请填写用户名',
    'auth.passwordRequired': '请填写密码',
    'auth.passwordOptional': '密码',
    'auth.restrictedModeHint': '不填写密码将以受限模式登录，部分功能（如修改密码、上传数据等）不可用。',
    'auth.failedLogin': '登录失败',
    'auth.failedRegister': '注册失败',
    'auth.successLogin': '登录成功',
    'auth.successRegister': '注册成功，请登录',
    'auth.loggedIn': '已登录',
    'auth.greeting': '出刀吧！',
    'auth.conflictTitle': '检测到云端冲突',
    'auth.conflictDesc': '本地账号与云端账号不一致，请选择处理方式。',
    'auth.conflictUseLocal': '本地上传到云',
    'auth.conflictUseCloud': '云覆盖本地',
    'auth.conflictLogout': '退出登录',

    // Settings & Security
    'settings.menu': '用户设置',
    'settings.title': '设置',
    'settings.profile': '个人资料',
    'settings.username': '用户名',
    'settings.usernameHelp': '修改用户名可能需要重新登录',
    'settings.changeUsernameSuccess': '用户名修改成功',
    'settings.changeUsernameFailed': '修改用户名失败',
    'settings.usernameTooShort': '用户名至少需要3个字符',
    'settings.avatar': '头像',
    'settings.avatarHelp': '选择一个头像并保存',
    'settings.avatarRequired': '请选择头像',
    'settings.changeAvatarSuccess': '头像修改成功',
    'settings.changeAvatarFailed': '头像修改失败',
    'settings.language': '语言',
    'settings.pwdRequired': '请填写当前密码和新密码',
    'settings.pwdMismatch': '两次新密码输入不一致',
    'settings.pwdTooShort': '新密码长度不能少于6位',
    'settings.changePwdFailed': '修改密码失败',
    'settings.changePwdSuccess': '密码修改成功',
    'settings.deleteAccountFailed': '注销账号失败',
    'settings.deleteAccountSuccess': '账号已注销',
    'settings.security': '账号安全',
    'settings.currentPwd': '当前密码',
    'settings.newPwd': '新密码',
    'settings.confirmPwd': '确认新密码',
    'settings.updatePwd': '修改密码',
    'settings.dangerZone': '危险区域',
    'settings.deleteDesc': '注销账号将永久删除您的所有数据，此操作无法撤销。',
    'settings.deleteAccount': '注销账号',
    'settings.deleteConfirmTitle': '确认注销账号？',
    'settings.deleteConfirmContent': '此操作无法撤销，您的所有数据将被永久删除。是否继续？',
    'settings.loginRequired': '请登录以访问账号安全设置',
    'settings.restrictedNoEdit': '受限模式下无法访问账号设置，请使用完整密码登录。',
    'settings.restrictedMode': '受限模式',
    'settings.restrictedModeDesc': '设置一个独立的受限密码，让他人可用该密码登录并查看数据，但无法修改密码、上传数据等敏感操作。留空受限密码并保存可关闭受限模式。',
    'settings.restrictedPwd': '受限密码（留空则关闭受限模式）',
    'settings.restrictedPwdRequired': '请填写受限密码',
    'settings.restrictedDisableHint': '不填写受限密码即为关闭受限模式',
    'settings.restrictedPwdSet': '受限密码设置成功',
    'settings.restrictedPwdDisabled': '受限模式已关闭',
    'settings.setRestrictedPwdFailed': '设置受限密码失败',
    'settings.currentPwdRequired': '请填写当前密码',
    'user.feedback': '交流/反馈',
    
    'common.back': '返回',
    'common.confirm': '确认',
    'common.cancel': '取消',
    'common.edit': '编辑',
    'common.delete': '删除',
    'common.save': '保存',
    'common.add': '新建',
    'common.copy': '复制',
    'common.replace': '替换',

    // Analyse Page
    langLabel: '中文',

    // 页面切换
    'page.analysis': '账号分析',
    'page.unionRaid': '联盟突袭',

    // 账号列表
    'accountList.label': '账号列表',
    'accountList.default': '默认账号列表',
    'accountList.unnamed': '未命名账号列表',
    'accountList.uploadExcel': '上传',
    'accountList.uploadExcelTitle': '上传 ExiaInvasion 导出的 Excel',
    'accountList.deleteLocal': '删除',
    'accountList.deleteLocalTitle': '删除本地列表',
    'accountList.importSuccess': '已成功导入 {count} 个本地账号',
    'accountList.importExcelFailed': '解析 Excel 失败',

    // 分析页
    accountAnalysis: '账号分析',
    account: '账号',
    synchro: '同步器',
    character: '角色',
    aelLabel: '攻优突破分',
    aelAbbr: 'AEL',
    'ael.tooltip': '攻优突破分(AEL)\nAEL = (1 + 0.9 × 攻击词条) × (1 + 10% + 优越词条) × (1 + 3% × 极限突破 + 2% × 核心强化)',
    strength: '综合强度',
    damage: '伤害',
    baseline: '基线',
    setBaseline: '设为基线',
    baselineDamage: '基线伤害',
    sortSynchroTip: '点击按同步器排序',
    sortDamageTip: '点击按伤害排序',

    // 侧栏/构建
    teamBuilder: '队伍构建',
      'layout.collapseSidebar': '收起侧栏',
      'layout.expandSidebar': '展开侧栏',

    // 联盟突袭统计
    'unionRaid.title': '联盟突袭统计',
    'unionRaid.difficulty.normal': '普通',
    'unionRaid.difficulty.hard': '困难',
    'unionRaid.copyTeam': '复制队伍到构建器',
    'unionRaid.copyPlanTeam': '复制规划队伍到构建器',
    'unionRaid.pastePlanTeam': '从构建器粘贴到规划',
    'unionRaid.pastePlanTeamEmpty': '构建器中没有可粘贴的队伍',
    'unionRaid.pastePlanTeamSuccess': '已将构建器队伍粘贴到规划',
    'unionRaid.teamCopied': '队伍已复制到构建器',
    'unionRaid.index': '序号',
    'unionRaid.remaining': '剩余',
    'unionRaid.remainingCount': '刀数',
    'unionRaid.strikes': '刀',
    'unionRaid.strike1': '第1刀',
    'unionRaid.strike2': '第2刀',
    'unionRaid.strike3': '第3刀',
    'unionRaid.boss': 'Boss',
    'unionRaid.squad': '队伍',
    'unionRaid.filter.level': '等级',
    'unionRaid.filter.step': 'Boss',
    'unionRaid.filter.all': '全部',
    'unionRaid.filter.stats.total': '已出刀数：实际 {actual} 刀 / 规划 {plan} 刀',
    'unionRaid.filter.stats.filtered': '符合条件的刀数：实际 {actual} 刀 / 规划 {plan} 刀',
    'unionRaid.noData': '暂无联盟突袭数据',
    'unionRaid.noCookieOrArea': '缺少 cookie 或 area_id 信息',
    'unionRaid.fetchError': '获取联盟突袭数据失败',
    'unionRaid.fetchRetry': '获取数据失败，{seconds}秒后重新获取',
    'unionRaid.refresh': '立即刷新',
    'unionRaid.exportJson': '导出规划 JSON',
    'unionRaid.exportSuccess': 'JSON 导出成功',
    'unionRaid.exportNoData': '暂无账号可导出',
    'unionRaid.exportMissingUidOnly': '所有账号都缺少 game_uid，无法导出',
    'unionRaid.exportSkippedMissingUid': '部分账号缺少 game_uid，已跳过',

    // Cloud Sync
    'unionRaid.cloudUpload': '上传到云端',
    'unionRaid.cloudDownload': '从云端下载',
    'unionRaid.cloudUploadSuccess': '云端同步成功',
    'unionRaid.cloudUploadFailed': '云端同步失败',
    'unionRaid.cloudDownloadSuccess': '云端数据加载成功',
    'unionRaid.cloudDownloadFailed': '云端下载失败',
    'unionRaid.cloudNoData': '云端暂无数据',
    'unionRaid.loginRequired': '请先登录以使用云端功能',
    'unionRaid.importJson': '导入规划 JSON',
    'unionRaid.importEmpty': '文件中未找到有效的规划数据',
    'unionRaid.importNoMatches': '文件中的规划未匹配任何账号',
    'unionRaid.importSuccess': '规划导入成功',
    'unionRaid.importPartialWarning': '部分规划未匹配任何账号',
    'unionRaid.importFailed': '规划 JSON 导入失败',
    'unionRaid.plan.actualLabel': '实际',
    'unionRaid.plan.planLabel': '规划',
    'unionRaid.plan.addCharacter': '添加角色',
    'unionRaid.plan.Edit': '编辑',
    'unionRaid.plan.noPlan': '尚未规划队伍',
    'unionRaid.plan.characterLimit': '最多选择5名角色',
    'unionRaid.plan.predictedDamage': '预测伤害',
    'unionRaid.plan.none': '无',
    'unionRaid.plan.defaultPrefix': '规划',
    'unionRaid.defaultPlanUndeletable': '默认规划不可删除',
    'unionRaid.realtimeSyncFailed': '实时同步失败',

    // 模板（Template）相关
    'tpl.import': '导入模板',
    'tpl.export': '导出模板',
    'tpl.save': '保存',
    'tpl.create': '新建',
    'tpl.copy': '复制',
    'tpl.rename': '重命名',
    'tpl.delete': '删除',
    'tpl.notSelected': '未选择',
    'tpl.inputName': '输入模板名称',
    'tpl.imported': '已导入 {count} 个模板',
    'tpl.importFailed': '导入模板失败：文件格式不正确',
    'tpl.exportFailed': '导出模板失败',
    'tpl.importFileAria': '导入模板文件',
    'tpl.defaultName': '默认模板',
    'tpl.defaultTemplateName': '默认模板',
    'tpl.defaultNamePrefix': '模板',
    'tpl.temporaryCopyName': '临时复制模板',
    'tpl.localOnlyBadge': '仅本地',
    'tpl.conflictCopySuffix': '（冲突副本）',
    'tpl.saveAsNew': '保存为新模板',

    // 上传
    'upload.title': '上传账号 JSON',
    'upload.helpLink': '如何获取 JSON 文件？',
    'upload.uploading': '正在上传...',
    'upload.retry': '重新上传',
    'upload.hint': '点击或拖拽上传',
    'upload.onlyOneJson': '仅支持单个 JSON（多账号）',
    'upload.onlyJson': '请上传 JSON 文件',
    'upload.readError': '文件读取失败',
    'upload.parseError': '文件解析失败',
    'common.remove': '移除',
    'common.error': '错误',

    // 筛选弹窗（部分）
    'filter.selectCharacter': '选择妮姬',
    'filter.name': '角色名称',
    'filter.searchPlaceholder': '搜索角色名称...',
    'filter.class': '职业',
    'filter.element': '代码',
    'filter.burst': '阶段',
    'filter.corporation': '企业',
    'filter.weapon': '武器类型',
    'filter.all': '全部',
    'filter.results': '筛选结果',
    'filter.selectedCharactersLabel': '已选择 {count}/{max} 人物',
    'filter.selectedEmpty': '尚未选择人物',
    'filter.selectedTag': '已选择',
    'filter.confirmSelection': '确认选择',
    'filter.loading': '加载角色数据中...',
    'filter.notFound': '未找到匹配的角色',
    'filter.cancel': '取消',
    'filter.choose': '选择',

    // 角色卡片（部分）
    'card.add': '添加角色',
    'card.totalCoeff': '总系数',
    'card.ael': '攻优突破分(AEL)',
    'card.strength': '综合强度',
    'card.collapse': '收起详情',
    'card.expand': '展开详情',
    'card.base': '基线',
    'card.target': '目标',
    'card.axis': '属性系数',
    'card.axisAtk': '基础攻击',
    'card.axisDef': '基础防御',
    'card.axisHP': '基础生命',
    'card.entryCoeff': '词条系数',

    // 词条/属性标签
    'stat.elementAdvantage': '优越',
    'stat.atkElemLbScore': 'AEL',
    'stat.attack': '攻击',
    'stat.ammo': '装弹',
    'stat.chargeSpeed': '蓄速',
    'stat.chargeDamage': '蓄伤',
    'stat.critical': '暴击',
    'stat.criticalDamage': '暴伤',
    'stat.hit': '命中',
    'stat.defense': '防御',
    'stat.hp': '生命',

    // 下拉选项
    'option.class.Attacker': '火力型',
    'option.class.Defender': '防御型',
    'option.class.Supporter': '支援型',
    'option.element.Electronic': '电击',
    'option.element.Fire': '燃烧',
    'option.element.Wind': '风压',
    'option.element.Water': '水冷',
    'option.element.Iron': '铁甲',
    'option.burst.Step1': 'I阶段',
    'option.burst.Step2': 'II阶段',
    'option.burst.Step3': 'III阶段',
    'option.burst.AllStep': '全阶段',
    'option.corporation.ELYSION': '极乐净土',
    'option.corporation.MISSILIS': '米西利斯',
    'option.corporation.TETRA': '泰特拉',
    'option.corporation.PILGRIM': '朝圣者',
    'option.corporation.ABNORMAL': '反常',
    'option.weapon.AR': 'AR',
    'option.weapon.SMG': 'SMG',
    'option.weapon.SG': 'SG',
    'option.weapon.SR': 'SR',
    'option.weapon.MG': 'MG',
  'option.weapon.RL': 'RL',
  },
  en: {
    // General
    appTitle: 'ExiaAnalysis',
    zh: 'Chinese',
    en: 'English',
    'lang.zh': 'Chinese',
    'lang.en': 'English',

    // Auth
    'auth.login': 'Login',
    'auth.logout': 'Logout',
    'auth.logoutSuccess': 'Logged out',
    'auth.register': 'Register',
    'auth.titleLogin': 'Login',
    'auth.titleRegister': 'Register',
    'auth.username': 'Username',
    'auth.password': 'Password',
    'auth.submitLogin': 'Login',
    'auth.submitRegister': 'Register',
    'auth.switchToRegister': 'Create account',
    'auth.switchToLogin': 'Back to login',
    'auth.cancel': 'Cancel',
    'auth.required': 'Please enter username and password',
    'auth.usernameRequired': 'Please enter username',
    'auth.passwordRequired': 'Please enter password',
    'auth.passwordOptional': 'Password (Optional, for restricted mode)',
    'auth.restrictedModeHint': 'Leaving password empty will log in with restricted mode. Some features (e.g. changing password, uploading data) will be unavailable.',
    'auth.failedLogin': 'Login failed',
    'auth.failedRegister': 'Registration failed',
    'auth.successLogin': 'Login successful',
    'auth.successRegister': 'Registration successful, please log in',
    'auth.loggedIn': 'Logged in',
    'auth.greeting': 'Time to Strike!',
    'auth.conflictTitle': 'Cloud conflict detected',
    'auth.conflictDesc': 'Local accounts differ from cloud data. Choose how to proceed.',
    'auth.conflictUseLocal': 'Upload local to cloud',
    'auth.conflictUseCloud': 'Overwrite local with cloud',
    'auth.conflictLogout': 'Log out',

    // Settings & Security
    'settings.menu': 'User Settings',
    'settings.title': 'Settings',
    'settings.profile': 'Profile',
    'settings.username': 'Username',
    'settings.usernameHelp': 'Changing username may require re-login',
    'settings.changeUsernameSuccess': 'Username changed successfully',
    'settings.changeUsernameFailed': 'Failed to change username',
    'settings.usernameTooShort': 'Username must be at least 3 characters',
    'settings.avatar': 'Avatar',
    'settings.avatarHelp': 'Choose an avatar and save',
    'settings.avatarRequired': 'Please select an avatar',
    'settings.changeAvatarSuccess': 'Avatar updated',
    'settings.changeAvatarFailed': 'Failed to update avatar',
    'settings.language': 'Language',
    'settings.pwdRequired': 'Please enter current and new password',
    'settings.pwdMismatch': 'New passwords do not match',
    'settings.pwdTooShort': 'Password must be at least 6 characters',
    'settings.changePwdFailed': 'Failed to change password',
    'settings.changePwdSuccess': 'Password changed successfully',
    'settings.deleteAccountFailed': 'Failed to delete account',
    'settings.deleteAccountSuccess': 'Account deleted successfully',
    'settings.security': 'Account Security',
    'settings.currentPwd': 'Current Password',
    'settings.newPwd': 'New Password',
    'settings.confirmPwd': 'Confirm New Password',
    'settings.updatePwd': 'Update Password',
    'settings.dangerZone': 'Danger Zone',
    'settings.deleteDesc': 'Deleting your account will permanently remove all data. This action cannot be undone.',
    'settings.deleteAccount': 'Delete Account',
    'settings.deleteConfirmTitle': 'Delete Account?',
    'settings.deleteConfirmContent': 'This action cannot be undone. All your data will be permanently lost. Continue?',
    'settings.loginRequired': 'Please login to access security settings',
    'settings.restrictedNoEdit': 'Account settings are unavailable in restricted mode. Please log in with your full password.',
    'settings.restrictedMode': 'Restricted Mode',
    'settings.restrictedModeDesc': 'Set a separate restricted password so others can log in and view data, but cannot change passwords, upload data, or perform other sensitive operations. Leave the restricted password empty and save to disable restricted mode.',
    'settings.restrictedPwd': 'Restricted Password (leave empty to disable)',
    'settings.restrictedPwdRequired': 'Please enter restricted password',
    'settings.restrictedDisableHint': 'Leave empty to disable restricted mode',
    'settings.restrictedPwdSet': 'Restricted password set successfully',
    'settings.restrictedPwdDisabled': 'Restricted mode disabled',
    'settings.setRestrictedPwdFailed': 'Failed to set restricted password',
    'settings.currentPwdRequired': 'Please enter current password',
    'user.feedback': 'Community / Feedback',

    'common.back': 'Back',
    'common.confirm': 'Confirm',
    'common.cancel': 'Cancel',
    'common.edit': 'Edit',
    'common.delete': 'Delete',
    'common.save': 'Save',
    'common.add': 'Create',
    'common.copy': 'Copy',
    'common.replace': 'Replace',

    // Analyse Page
    langLabel: 'EN',

    // Page switch
    'page.analysis': 'Account Analysis',
    'page.unionRaid': 'Union Raid',

    // Account list
    'accountList.label': 'Account List',
    'accountList.default': 'Default Account List',
    'accountList.unnamed': 'Unnamed Account List',
    'accountList.uploadExcel': 'Upload',
    'accountList.uploadExcelTitle': 'Upload an Excel exported from ExiaInvasion',
    'accountList.deleteLocal': 'Delete',
    'accountList.deleteLocalTitle': 'Delete local list',
    'accountList.importSuccess': 'Imported {count} local accounts successfully.',
    'accountList.importExcelFailed': 'Failed to parse Excel',
    'accountList.exportWithPlans': 'Export Plans',
    'accountList.exportWithPlansTitle': 'Export the current account list with union raid planning',
    'accountList.importNoAccounts': 'No account data found in the workbook',
    'accountList.importPlanningSuccess': 'Imported raid planning for {matched} accounts',
    'accountList.importPlanningPartial': 'Imported raid planning for {matched} accounts and skipped {unmatched} unmatched accounts',
    'accountList.importPlanningNoMatches': 'Uploaded raid planning did not match any current account',
    'accountList.exportNoData': 'No accounts available to export',
    'accountList.exportSuccess': 'Workbook exported successfully',
    'accountList.exportFailed': 'Failed to export workbook',

    // Analyzer
    accountAnalysis: 'Account Analysis',
    account: 'Account',
    synchro: 'Synchro',
    character: 'Character',
    aelLabel: 'AEL',
    aelAbbr: 'AEL',
    'ael.tooltip': 'Attack Element Limit Break Score (AEL)\nAEL = (1 + 0.9 × ATK%) × (1 + 10% + Elem%) × (1 + 3% × Limit Break + 2% × Core Refinement)',
    strength: 'Overall Strength',
    damage: 'Damage',
    baseline: 'Baseline',
    setBaseline: 'Set Baseline',
    baselineDamage: 'Baseline Damage',
    sortSynchroTip: 'Click to sort by Synchro',
    sortDamageTip: 'Click to sort by Damage',

    // Sidebar/Builder
    teamBuilder: 'Team Builder',
      'layout.collapseSidebar': 'Collapse Sidebar',
      'layout.expandSidebar': 'Expand Sidebar',

    // Union Raid Stats
    'unionRaid.title': 'Union Raid Statistics',
    'unionRaid.difficulty.normal': 'Normal',
    'unionRaid.difficulty.hard': 'Hard',
    'unionRaid.copyTeam': 'Copy Team to Builder',
    'unionRaid.copyPlanTeam': 'Copy Planned Team to Builder',
    'unionRaid.pastePlanTeam': 'Paste Builder Team into Plan',
    'unionRaid.pastePlanTeamEmpty': 'No builder team available to paste',
    'unionRaid.pastePlanTeamSuccess': 'Builder team pasted into plan',
    'unionRaid.teamCopied': 'Team copied to builder',
    'unionRaid.index': '#',
    'unionRaid.remaining': ' Remaining',
    'unionRaid.remainingCount': 'Count',
    'unionRaid.strikes': 'Left',
    'unionRaid.strike1': 'Strike 1',
    'unionRaid.strike2': 'Strike 2',
    'unionRaid.strike3': 'Strike 3',
    'unionRaid.boss': 'Boss',
    'unionRaid.squad': 'Squad',
    'unionRaid.filter.level': 'Level',
    'unionRaid.filter.step': 'Boss',
    'unionRaid.filter.all': 'All',
    'unionRaid.filter.stats.total': 'Total strikes: Actual {actual} / Planned {plan}',
    'unionRaid.filter.stats.filtered': 'Matching strikes: Actual {actual} / Planned {plan}',
    'unionRaid.noData': 'No union raid data available',
    'unionRaid.noCookieOrArea': 'Missing cookie or area_id',
    'unionRaid.fetchError': 'Failed to fetch union raid data',
    'unionRaid.fetchRetry': 'Failed to fetch data, retrying in {seconds}s',
    'unionRaid.refresh': 'Refresh now',
    'unionRaid.exportJson': 'Export planning JSON',
    'unionRaid.exportSuccess': 'JSON exported successfully',
    'unionRaid.exportFailed': 'Failed to export planning JSON',
    'unionRaid.exportNoData': 'No accounts available to export',
    'unionRaid.exportMissingUidOnly': 'All accounts are missing game_uid, nothing to export',
    'unionRaid.exportSkippedMissingUid': 'skipped missing game_uid',

    // Cloud Sync
    'unionRaid.cloudUpload': 'Save to Cloud',
    'unionRaid.cloudDownload': 'Load from Cloud',
    'unionRaid.cloudUploadSuccess': 'Cloud sync successful',
    'unionRaid.cloudUploadFailed': 'Cloud sync failed',
    'unionRaid.cloudDownloadSuccess': 'Cloud data loaded',
    'unionRaid.cloudDownloadFailed': 'Cloud download failed',
    'unionRaid.cloudNoData': 'No data in cloud',
    'unionRaid.loginRequired': 'Please login to use cloud features',
    'unionRaid.importJson': 'Import planning JSON',
    'unionRaid.importEmpty': 'No valid planning entries found in the file',
    'unionRaid.importNoMatches': 'No planning entries matched the loaded accounts',
    'unionRaid.importSuccess': 'Planning imported successfully',
    'unionRaid.importPartialWarning': 'Some entries did not match any loaded account',
    'unionRaid.importFailed': 'Failed to import planning JSON',
    'unionRaid.plan.actualLabel': 'Actual',
    'unionRaid.plan.planLabel': 'Planning',
    'unionRaid.plan.addCharacter': 'Add Character',
    'unionRaid.plan.Edit': 'Edit',
    'unionRaid.plan.noPlan': 'No planned squad yet',
    'unionRaid.plan.characterLimit': 'Up to 5 characters',
    'unionRaid.plan.predictedDamage': 'Estimated Damage',
    'unionRaid.plan.none': 'None',
    'unionRaid.plan.defaultPrefix': 'Plan',
    'unionRaid.defaultPlanUndeletable': 'Default plan cannot be deleted',
    'unionRaid.realtimeSyncFailed': 'Realtime sync failed',

    // Template related
    'tpl.import': 'Import Templates',
    'tpl.export': 'Export Templates',
    'tpl.save': 'Save',
    'tpl.create': 'Create',
    'tpl.copy': 'Copy',
    'tpl.rename': 'Rename',
    'tpl.delete': 'Delete',
    'tpl.notSelected': 'Not selected',
    'tpl.inputName': 'Enter template name',
    'tpl.imported': 'Imported {count} templates',
    'tpl.importFailed': 'Import failed: invalid file format',
    'tpl.exportFailed': 'Export failed',
    'tpl.importFileAria': 'Import template file',
    'tpl.defaultName': 'Default Template',
    'tpl.defaultTemplateName': 'Default Template',
    'tpl.defaultNamePrefix': 'Template',
    'tpl.temporaryCopyName': 'Temporary Copy',
    'tpl.localOnlyBadge': 'Local Only',
    'tpl.conflictCopySuffix': ' (Conflict Copy)',
    'tpl.saveAsNew': 'Save as new template',

    // Upload
    'upload.title': 'Upload Accounts JSON',
    'upload.helpLink': 'How to get the JSON file?',
    'upload.uploading': 'Uploading...',
    'upload.retry': 'Re-upload',
    'upload.hint': 'Click or drag to upload',
    'upload.onlyOneJson': 'Only one JSON (multi-accounts) supported',
    'upload.onlyJson': 'Please upload a JSON file',
    'upload.readError': 'File reading failed',
    'upload.parseError': 'Parse failed',
    'common.remove': 'Remove',
    'common.error': 'Error',

    // Filter dialog (partial)
    'filter.selectCharacter': 'Select Nikke',
    'filter.name': 'Character Name',
    'filter.searchPlaceholder': 'Search character name...',
    'filter.class': 'Class',
    'filter.element': 'Element',
    'filter.burst': 'Burst Stage',
    'filter.corporation': 'Manufacturer',
    'filter.weapon': 'Weapon Type',
    'filter.all': 'All',
    'filter.results': 'Results',
    'filter.selectedCharactersLabel': 'Selected {count}/{max} characters',
    'filter.selectedEmpty': 'No characters selected yet',
    'filter.selectedTag': 'Selected',
    'filter.confirmSelection': 'Confirm Selection',
    'filter.loading': 'Loading characters...',
    'filter.notFound': 'No matching characters',
    'filter.cancel': 'Cancel',
    'filter.choose': 'Select',

    // Character card (partial)
    'card.add': 'Add Character',
    'card.totalCoeff': 'Total Coeff',
    'card.ael': 'AEL',
    'card.strength': 'Overall Strength',
    'card.collapse': 'Collapse',
    'card.expand': 'Expand',
    'card.base': 'Baseline',
    'card.target': 'Target',
    'card.axis': 'Attribute Coeff',
    'card.axisAtk': 'Base Attack',
    'card.axisDef': 'Base Defense',
    'card.axisHP': 'Base HP',
    'card.entryCoeff': 'Entry Coeff',

    // Stat labels
    'stat.elementAdvantage': 'Elem',
    'stat.atkElemLbScore': 'AEL',
    'stat.attack': 'Atk',
    'stat.ammo': 'Ammo',
    'stat.chargeSpeed': 'Chg Spd',
    'stat.chargeDamage': 'Chg DMG',
    'stat.critical': 'Crit%',
    'stat.criticalDamage': 'Crit DMG',
    'stat.hit': 'Hit%',
    'stat.defense': 'Def',
    'stat.hp': 'HP',

    // Dropdown options
    'option.class.Attacker': 'Attacker',
    'option.class.Defender': 'Defender',
    'option.class.Supporter': 'Supporter',
    'option.element.Electronic': 'Electronic',
    'option.element.Fire': 'Fire',
    'option.element.Wind': 'Wind',
    'option.element.Water': 'Water',
    'option.element.Iron': 'Iron',
    'option.burst.Step1': 'Step I',
    'option.burst.Step2': 'Step II',
    'option.burst.Step3': 'Step III',
    'option.burst.AllStep': 'All Steps',
    'option.corporation.ELYSION': 'ELYSION',
    'option.corporation.MISSILIS': 'MISSILIS',
    'option.corporation.TETRA': 'TETRA',
    'option.corporation.PILGRIM': 'PILGRIM',
    'option.corporation.ABNORMAL': 'ABNORMAL',
    'option.weapon.AR': 'AR',
    'option.weapon.SMG': 'SMG',
    'option.weapon.SG': 'SG',
    'option.weapon.SR': 'SR',
    'option.weapon.MG': 'MG',
  'option.weapon.RL': 'RL',
  }
}

export default TRANSLATIONS
